...
const searchengine = require("../utils/libs/search.min.js")
...
exports.createRoom = async (req, res) => {
    ...
    const room_ = await room.findOne({ creater: creater, user: data.user })
    if (!room_) {
...
exports.getRooms = async(req, res)=>{
    const rooms = await room.find({
        "$or":[{creater: req.userInfo._id}, {user: req.userInfo._id}]
    }).populate('user').populate('creater')
